...
const WorkDay = ({route, navigation}) => {
  const [userData, setUserdata] = React.useState('');
  const [projects, setProjects] = React.useState('');
  const [checkedList, setcheckedList] = React.useState({});
  const [startdate, setStartDate] = React.useState(new Date());
  const [enddate, setEndDate] = React.useState(new Date());
  const [secondStartDate, setsecondStartDate] = React.useState('');
  const [secondEndDate, setSecondEndDate] = React.useState('');
  ...
  const submitUpdate = () => {
    ...
    axios({
      method: 'post',
      url: 'https://hassmalie.herokuapp.com/api/reports/',
      data: formData,
    })
      .then(Res => {
